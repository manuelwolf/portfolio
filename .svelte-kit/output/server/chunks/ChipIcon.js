...
import { t as theme } from "./skills.params.js";
...
const ChipIcon = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_theme();
  return `${((tag) => {
    return tag ? `<${href ? "a" : "div"}${add_attribute("href", href, 0)} class="${escape(null_to_empty(`chip-icon row-center relative text-inherit decoration-none p-10px m-r-5px m-b-5px border-1px border-solid border-[var(--border)] hover:border-[var(--border-hover)] rounded-10px ${href ? "cursor-pointer" : "cursor-help"} ${grayscale ? "grayscale-65 hover:grayscale-0" : ""}`), true) + " svelte-1m94jop"}"${add_attribute("data-help", name, 0)}>${is_void(tag) ? "" : `${$$slots.default ? `${slots.default ? slots.default({}) : ``}` : `<img class="${[
